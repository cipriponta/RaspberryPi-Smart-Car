...
class ChassisDriver():
    ...
    def change_direction(self, line_shift):
        self.pid_control(line_shift)
    ...
    def pid_control(self, line_shift):
        ...
        self.error = MOTOR_PID_LINE_SETPOINT - self.input
    ...
    def calculate_duty_cycles(self):
        ...
        if self.left_motor_duty_cycle < 0:
            self.left_motor_duty_cycle = 0
        elif self.left_motor_duty_cycle > 100:
            self.left_motor_duty_cycle = 100
        
        if self.right_motor_duty_cycle < 0:
            self.right_motor_duty_cycle = 0
        elif self.right_motor_duty_cycle > 100:
            self.right_motor_duty_cycle = 100
